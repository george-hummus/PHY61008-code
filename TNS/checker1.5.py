...
for lID in tqdm(localIDs): #swap around so sees if IDs in the local database are in the offical one
    index = np.where(IDs==lID)[0] #looks for index that matches the ID
    if index.size == 0: #if no index produced then there is no match
        nomatch.append(lID)
    elif index.size == 1: #if multiple one index then there has been a match
        match.append(lID)
