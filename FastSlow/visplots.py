...
for j, fname in enumerate(lists):
    ...
    ax[j].plot(times,np.zeros(len(times)),color="grey",alpha=0.5,zorder=0) #horizon
    ax[j].plot(times,35*np.ones(len(times)),color="grey",alpha=0.5,zorder=0) #lower alt limit
    ...
    ax[j].set_ylim(0,90)
    ax[j].set_title(f"Visibility of top 5 targets from {fname} during dark time on La Palma ({today.strftime('%Y-%m-%d')})")
...
#save
plt.tight_layout()
plt.savefig(f"VisPlots/top5_{today.strftime('%Y%m%d')}.jpg",dpi=600)
plt.close()
